...
selected_categories = st.sidebar.multiselect(
    "Select Product Categories", options=unique_categories, default=unique_categories
)

selected_payment_methods = st.sidebar.multiselect(
    "Select Payment Methods", options=unique_payment_methods, default=unique_payment_methods
)
...
with col2:
    ...
    discounted_transactions = df_all[df_all["discount_percent"] > 0].shape[0]
    discount_percentage = (discounted_transactions / total_transactions * 100) if total_transactions > 0 else 0
    st.metric("Discount Usage", f"{discount_percentage:.1f}%")
    st.caption(f"{discounted_transactions} of {total_transactions} transactions had a discount.")
...
if not df_revenue.empty:
    fig_revenue = px.line(
        df_revenue, x="day", y="order_total",
        title="Daily Revenue",
        labels={"day": "Date", "order_total": "Revenue ($)"}
    )
...
if not df_filtered.empty:
    ...
    fig_product = px.bar(
        df_product, x="product_id", y="total_revenue", color="product_category",
...
with col_cat:
    ...
    if not df_filtered.empty:
        ...
        fig_category = px.bar(
            df_category, x="product_category", y="revenue",
...
with col_pay:
    ...
    if not df_filtered.empty:
        ...
        fig_payment = px.pie(
            df_payment, names="payment_method", values="count",
